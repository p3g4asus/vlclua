...
function parse()
    while true do
        ...
        if line==nil then 
            ...
            pth = string.match(line,"var vS *= *'([^']+)';")
            if pth then
                return { { path = pth, title = "Diretta La7" }}
            end
        end
    end
    vlc.msg.err( "Couldn't extract la7 video URL, please check for updates to this script" )
    return {}
end

-- Probe function.
function probe()
    return string.match( vlc.path, "la7%.it/dirette%-tv" )
